...
export class RechercheParTypesComponent implements OnInit {
  ...
  idTyp!:number;
  type!:Type[];
  constructor(public authservice: AuthService , public motorsService: MotorsService) { }

  ngOnInit(): void {
    
    this.type=this.motorsService.listeTypes();
    
    
  }
  onChange(){
    console.log(this.idTyp);
    this.motors = this.motorsService.rechercherParType(this.idTyp)
  }
